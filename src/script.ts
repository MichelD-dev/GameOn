...
const isQuantityValid = () => {
   ...
   */
  if (
    DOM.quantityInput.valueAsNumber < 0 ||
...
const isLocationChecked = () => {
   ...
   */
  if (
    DOM.LocationsList.filter(location => (location as HTMLInputElement).checked)
      .length === 0
  ) {
...
const validate = (e: SubmitEvent | KeyboardEvent) => {
  ...
  if (
    ...
    let userAge = Math.ceil(
      calcAge / (1000 * 3600 * 24) / 365
    ...
    DOM.LocationsList.map(
      location => ((location as HTMLInputElement).checked = false)
    )
